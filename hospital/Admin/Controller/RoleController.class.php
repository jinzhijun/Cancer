...
class RoleController extends AdminController{
	...
	function distribute($role_id){
		...
		if(!empty($_POST)){
			...
			//dump($_POST); //role_id   auth_id(数组)
			//收集好设置的权限，并"制作"为数据表要求格式并"存储"
		...
		//查询当前角色已经拥有的权限
		$have_authids=$role_info['role_auth_ids'];   //例如：101  103  105 107
		$have_authids=explode(',', $have_authids);   //字符串变为数组，使得判断更加严谨
		//dump($have_authids);
